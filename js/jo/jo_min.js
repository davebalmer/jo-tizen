...
console.log(strings.join(" "));};Function.prototype.extend=function(superclass,proto){if(typeof Object.create!=="undefined")
this.prototype=Object.create(superclass.prototype);else
this.prototype=new superclass();if(proto){for(var i in proto)
...
console.log=function(msg){};jo={platform:"webkit",version:"0.5.0",useragent:['ipad','iphone','ipod','playbook','bb10','webos','hpwos','bada','ouya','tizen','android','kindle','silk','iemobile','msie','opera','chrome','safari','mozilla','gecko'],osMap:{ipad:"ios",iphone:"ios",ipod:"ios",webos:"webos",hpwos:"webos",silk:"android",ouya:"android",kindle:"android",msie:"windows",iemobile:"windows",safari:"osx"},debug:false,setDebug:function(state){this.debug=state;},flag:{stopback:false},load:function(call,context){joDOM.enable();this.loadEvent=new joSubject(this);this.unloadEvent=new joSubject(this);document.body.onMouseDown=function(e){e.preventDefault();};document.body.onDragStart=function(e){e.preventDefault();};if(typeof navigator=='object'&&navigator.userAgent){var agent=navigator.userAgent.toLowerCase();for(var i=0;i<this.useragent.length;i++){if(agent.indexOf(this.useragent[i])>=0){this.platform=this.useragent[i];break;}}}
this.os=this.osMap[this.platform]||this.platform;if(joEvent){var o=document.createElement('div');var test=("ontouchstart"in o);if(!test){o.setAttribute("ontouchstart",'return;');test=(typeof o.ontouchstart==='function');}
joEvent.touchy=test;o=null;}
if(joGesture)
joGesture.load();var s=(typeof joScroller!=="undefined")?joScroller.prototype:null;var d=joDOM;if(s&&this.matchPlatform("tizen msie chrome safari bb10")){joDOM.addCSSClass(document.body,"nativescroll");s.onDown=function(){};s.setEvents=function(){};s.onUp=function(){};s.onMove=function(){};s.onClick=function(){};s.setPosition=s.setPositionNative;}
if(typeof document.body.style.webkitTransition!=="undefined"){joEvent.map.transitionend="webkitTransitionEnd";d.transform=function(node,arg){node.style.webkitTransform=arg;};d.transformOrigin=function(node,arg){node.style.webkitTransformOrigin=arg;};d.transition=function(node,arg){node.style.webkitTransition=arg;};}
else if(typeof document.body.style.MozTransition!=="undefined"){if(s)s.setPosition=function(x,y,node){node.style.MozTransform="translate("+x+"px,"+y+"px)";};d.transform=function(node,arg){node.style.MozTransform=arg;};d.transformOrigin=function(node,arg){node.style.MozTransformOrigin=arg;};d.transition=function(node,arg){node.style.MozTransition=arg;};}
else if(typeof document.body.style.msTransform!=="undefined"){if(s)s.setPosition=function(x,y,node){node.style.msTransform="translate("+x+"px,"+y+"px)";};d.transform=function(node,arg){node.style.msTransform=arg;};d.transformOrigin=function(node,arg){node.style.msTransformOrigin=arg;};d.transition=function(node,arg){node.style.msTransition=arg;};}
else if(typeof document.body.style.OTransition!=="undefined"){if(s)s.setPosition=function(x,y,node){node.style.OTransform="translate("+x+"px,"+y+"px)";};joEvent.map.transitionend="otransitionend";d.transform=function(node,arg){node.style.OTransform=arg;};d.transformOrigin=function(node,arg){node.style.OTransformOrigin=arg;};d.transition=function(node,arg){node.style.OTransition=arg;};}
else{s.velocity=0;s.bump=0;if(s)s.setPosition=function(x,y,node){if(this.vertical)
node.style.top=y+"px";if(this.horizontal)
node.style.left=x+"px";};}
if(!window.requestAnimationFrame){console.log("jo: swapping requestanimationframe with settimeout");jo.requestAnimationFrame=false;window.requestAnimationFrame=function(call){setTimeout(call,17);}}
else{jo.requestAnimationFrame=true;}
...
document.body.appendChild(css);return css;},applyCSS:function(style,oldnode){var css=oldnode||joDOM.create('jostyle');css.innerHTML='<style>'+style+'</style>';if(!oldnode)
document.body.appendChild(css);return css;},addMeta:function(name,content){var meta=joDOM.create("meta");meta.setAttribute("name",name);meta.setAttribute("content",content);document.head.appendChild(meta);},pageOffsetLeft:function(node){var l=0;while(typeof node!=='undefined'&&node&&node.parentNode!==window){if(node.offsetLeft)
...
return t;},getBounds:function(node){var top=joDOM.pageOffsetTop(node);var left=joDOM.pageOffsetLeft(node);var bottom=top+node.offsetHeight;var right=left+node.offsetWidth;return{top:top,left:left,bottom:bottom,right:right,center:{x:left+(right-left)/2,y:top+(bottom-top)/2}};},attach:function(node,parent){parent.appendChild(node);},detach:function(node,parent){if(!parent)
parent=node.parentNode;if(parent)
parent.removeChild(node);}};joCSSRule=function(data){this.setData(data);};joCSSRule.prototype={container:null,setData:function(data){this.data=data||"";this.enable();},clear:function(){this.setData();},disable:function(){joDOM.removeCSS(this.container);},enable:function(){this.container=joDOM.applyCSS(this.data,this.container);}};joEvent={eventMap:{"mousedown":"touchstart","mousemove":"touchmove","mouseup":"touchend","mouseout":"touchcancel"},map:{click:"click",transitionend:"transitionend"},touchy:false,getTarget:function(e){if(!e)
...
this.cache[key].view=null;return this;}};joClipboard={data:"",get:function(){return joPreference.get("joClipboardData")||this.data;},set:function(clip){this.data=clip;joPreference.set("joClipboardData");}};joQueue=function(){this.queue=[];this.doneEvent=new joSubject(this);};joQueue.prototype={push:function(call,delay,context,data){this.queue.push({delay:delay,call:call,context:context,data:data});},tick:function(){var t=(new Date()*1);if(this.next==0||t>=this.next){var c=this.queue.shift();if(c.context)
c.call.call(c.context,c.data||null);else
c.call(c.data||null);if(this.queue.length)
this.next=t+this.queue[0].delay;}
if(this.queue.length){var self=this;window.requestAnimationFrame(function(){self.tick.call(self);});}
else{this.stop();}},start:function(){this.working=true;this.next=0;if(this.queue.length){joDefer(this.tick,this);}
return this;},stop:function(){this.working=false;this.next=0;this.doneEvent.fire();},getElapsed:function(last){return(new Date()*1)-last;},getTime:function(){return(new Date()*1);},setLast:function(time){if(!time)
time=(new Date()*1);this.last=time;return this;}};joTimer=function(sec,resolution){this.setDuration(sec||60);this.expiredEvent=new joSubject(this);this.startEvent=new joSubject(this);this.stopEvent=new joSubject(this);this.updateEvent=new joSubject(this);this.resetEvent=new joSubject(this);this.resolution=resolution||100;this.init();this.started=0;this.paused=0;};joTimer.prototype={start:function(silent){var paused=this.paused;this.init();if(paused)
this.started=joTime.timestamp()-paused;else
this.started=joTime.timestamp();this.paused=0;var self=this;this.interval=setInterval(function(){joDefer(self.update,self);},this.resolution);if(!silent)
this.startEvent.fire();return this;},init:function(){if(this.interval)
clearInterval(this.interval);this.interval=null;this.lastRemaining=null;return this;},stop:function(silent){this.init();this.paused=joTime.timestamp()-this.started;if(!silent)
this.stopEvent.fire();return this;},pause:function(){if(this.interval){clearInterval(this.interval);this.interval=0;}},resume:function(){if(this.interval||this.paused)
return;var self=this;this.interval=setInterval(function(){joDefer(self.update,self);},this.resolution);},reset:function(silent){this.started=joTime.timestamp();this.paused=0;if(!silent)
this.resetEvent.fire();return this;},setDuration:function(sec){this.duration=sec*1000;return this;},update:function(){var remaining=Math.round((this.duration-(joTime.timestamp()-this.started))/1000);if(this.lastRemaining==null||remaining<this.lastRemaining)
this.updateEvent.fire(remaining);if(remaining<=0)
this.expired();this.lastRemaining=remaining;return this;},expired:function(){this.stop();this.expiredEvent.fire();}};joDataSource=function(data){this.changeEvent=new joSubject(this);this.errorEvent=new joSubject(this);if(typeof data!=="undefined")
...
return h[i];}
return null;}};joTemplate=function(template){this.setTemplate(template||"");};joTemplate.prototype={setTemplate:function(template){if(template&&typeof template=='object'&&template instanceof Array)
this.template=template.join(" ");else
this.template=template+"";return this;},apply:function(data){var str=this.template+"";for(var i in data){var key="__"+i+"__";var reg=new RegExp(key,"g");str=str.replace(reg,data[i]);}
return str;}};joInterface=function(parent){jo.initTagMap();return this.get(parent);};joInterface.prototype={get:function(parent){parent=joDOM.get(parent);if(!parent)
...
this.appendChild(newnode);var self=this;var transitionevent=null;joDefer(animate,this,20);function animate(){if(typeof window.onwebkittransitionend!=='undefined')
transitionevent=joEvent.on(newchild,joEvent.map.transitionend,cleanup,self);else
joDefer(cleanup,this,500);if(newclass&&newchild)
...
joEvent.remove(newchild,joEvent.map.transitionend,transitionevent);joDOM.removeCSSClass(newchild,"next");joDOM.removeCSSClass(newchild,"prev");}}
...
return this;this.data.push(o);this.index=this.data.length-1;this.draw();this.pushEvent.fire(o);this.captureBack();return this;},setLocked:function(state){this.locked=(state)?1:0;return this;},pop:function(){if(this.data.length>this.locked){var o=this.data.pop();this.index=this.data.length-1;this.draw();if(typeof o.deactivate==="function")
o.deactivate.call(o);if(!this.data.length)
this.hide();}
this.captureBack();if(this.data.length>0)
...
return this;this.data=[o];this.lastIndex=1;this.index=0;this.draw();this.captureBack();this.popEvent.fire();this.homeEvent.fire();}
...
return null;}}});joInput=function(data,placeholder){joControl.apply(this,arguments);if(placeholder)
this.setPlaceholder(placeholder);};joInput.extend(joControl,{tagName:"input",type:"text",setData:function(data){if(data!==this.data){this.data=data;if(typeof this.container.value!=="undefined")
this.container.value=data;else
this.container.innerHTML=data;this.changeEvent.fire(this.data);}
return this;},setPlaceholder:function(placeholder){if(typeof this.container!=="undefined")
this.container.setAttribute("placeholder",placeholder);},getData:function(){if(typeof this.container.value!=="undefined")
...
return;o.setAttribute("type",this.type);o.setAttribute("tabindex","1");o.contentEditable=this.enabled;return o;},setEvents:function(){if(!this.container)
...
callback();}}
return this;}});joShim=function(){this.showEvent=new joSubject(this);this.hideEvent=new joSubject(this);this.selectEvent=new joSubject(this);joContainer.apply(this,arguments);};joShim.extend(joContainer,{tagName:"joshim",setEvents:function(){joEvent.on(this.container,"mousedown",this.onMouseDown,this);},onMouseDown:function(e){joEvent.stop(e);this.hide();},hide:function(){joDefer(function(){joEvent.on(this.container,joEvent.map.transitionend,this.onHide,this);this.container.className='';},this);return this;},show:function(){this.attach();joEvent.remove(this.container,joEvent.map.transitionend,this.onHide,this);joDefer(function(){this.container.className='show';},this);if(!this.lastParent)
this.lastParent=document.body;return this;},onShow:function(){this.showEvent.fire();},onHide:function(){joEvent.remove(this.container,joEvent.map.transitionend,this.onHide,this);this.detach();this.hideEvent.fire();}});joSound=function(filename,repeat){this.endedEvent=new joSubject(this);this.errorEvent=new joSubject(this);if(typeof Audio=='undefined')
...
this.switchScroller();joStack.prototype.back.call(this);return this;},home:function(){if(this.data&&this.data.length&&this.data.length>1){this.switchScroller();joStack.prototype.home.call(this);}
...
this.button.innerHTML=this.labels[(this.data)?1:0];joDefer(function(){if(this.data)
joDOM.addCSSClass(this.container,"on");else
joDOM.removeCSSClass(this.container,"on");},this);}});joSlider=function(value){this.min=0;this.max=1;this.snap=0;this.range=1;this.thumb=null;this.horizontal=1;this.vertical=0;this.moved=false;this.jump=true;this.slideStartEvent=new joSubject();this.slideEndEvent=new joSubject();joControl.call(this,null,value);};joSlider.extend(joControl,{tagName:"joslider",setRange:function(min,max,snap){if(min>=max){joLog("WARNING: joSlider.setRange, min must be less than max.");return this;}
...
o.setAttribute("tabindex","1");var t=joDOM.create("josliderthumb");o.appendChild(t);this.thumb=t;return o;},onDown:function(e){joEvent.stop(e);this.reset();joDOM.addCSSClass(this.container,"live");var node=this.container.firstChild;this.inMotion=true;this.moved=false;this.slideStartEvent.fire(this.value);if(!this.mousemove){this.mousemove=joEvent.on(document.body,"mousemove",this.onMove,this);this.mouseup=joEvent.capture(document.body,"mouseup",this.onUp,this);}},reset:function(){this.moved=false;this.inMotion=false;this.firstX=-1;this.firstY=-1;},onMove:function(e){if(!this.inMotion)
...
x=0;else if(x>w)
x=w;if(!this.snap)
this.moveTo(x);this.setValue((x/w)*this.range+this.min,!this.snap);},moveTo:function(x){this.thumb.style.left=x+"px";return this;},initValue:function(value){if(!this.container)
...
this.ctx=this.canvas.getContext("2d");this.ctx.imageSmoothingEnabled=false;this.ctx.mozImageSmoothingEnabled=false;this.ctx.oImageSmoothingEnabled=false;this.ctx.webkitImageSmoothingEnabled=false;if(w||h)
this.setSize(w,h);}
joCanvas.prototype={getImage:function(){return this.canvas.toDataURL();},getCSSImage:function(){return"url("+this.canvas.toDataURL()+")";},clear:function(){this.ctx.clearRect(0,0,this.width,this.height);},setSize:function(w,h){if(!w)
w=50;if(!h)
h=50;this.canvas.setAttribute("width",w);this.canvas.setAttribute("height",h);this.width=w;this.height=h;return this;},getContext:function(){return this.ctx;},roundRect:function(sx,sy,ex,ey,r){var r2d=Math.PI/180;if((ex-sx)-(2*r)<0)
r=((ex-sx)/2);if((ey-sy)-(2*r)<0)
r=((ey-sy)/2);var ctx=this.ctx;ctx.beginPath();ctx.moveTo(sx+r,sy);ctx.lineTo(ex-r,sy);ctx.arc(ex-r,sy+r,r,r2d*270,r2d*360);ctx.lineTo(ex,ey-r);ctx.arc(ex-r,ey-r,r,0,r2d*90);ctx.lineTo(sx+r,ey);ctx.arc(sx+r,ey-r,r,r2d*90,r2d*180);ctx.lineTo(sx,sy+r);ctx.arc(sx+r,sy+r,r,r2d*180,r2d*270);ctx.closePath();return this;},setFont:function(font){this.ctx.font=font;return this;},ellipse:function(sx,sy,r){this.ctx.beginPath();this.ctx.arc(sx,sy,r,0,Math.PI*2);this.ctx.closePath();return this;},getNode:function(){return this.canvas;}};